...
var (
	...
	user    = config.RegisterString("services.postgres.user", "hesabfun", "postgres user")
	dbname  = config.RegisterString("services.postgres.db", "hesabfun", "postgres database")
	pass    = config.RegisterString("services.postgres.password", "hesabfun123", "postgres password")
